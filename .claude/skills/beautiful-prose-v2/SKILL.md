...
description: Writing style contract plus nine craft rules measured against professional feature journalism. Use when writing or rewriting any prose that ships on this site (post bodies, standfirsts, captions, standing boxes, recipe notes, page copy), and for reported features. Supersedes beautiful-prose. Carries rules on the reporter's presence, aphorism density, mechanism depth, numerals, and why a short piece is a piece that dropped its reporting.
---
...
**Why v2 exists.** Skills are cached on first load in a session, so an edited skill does not
take effect until the next one. This is a complete copy of `beautiful-prose` with the craft
rules folded in, published under a new name so it loads fresh. Prefer this over
`beautiful-prose`; the two are identical above the Project binding.
...
> ## Status: round one evidence, round two in progress
>
> Rules 5 to 11 come from **round one** of the exercise: three features written blind to a
> written-down theory of the form, then compared against three real articles. Rules 12 and 13
> are reader-caught rather than measured, and are labelled as such.
>
> Round two is under way. When it finishes, this section gets revised once, from the whole
> test, rather than edited piecemeal. An earlier version of these rules had to be thrown out
> because it drew six of seven examples from my own uncorrected drafts instead of from the
> comparison, which is the hazard of writing conclusions down while the experiment is open.
